...
df1 = pd.read_csv(io.StringIO(uploaded['leads-pipedrive.csv'].decode('utf-8')))
...
df2 = pd.read_csv(io.StringIO(uploaded2['leads-teste.csv'].decode('utf-8')))
...
#        dominios = [e for e in dominios_list if e not in ('gmail.com', 'hotmail.com', 'yahoo.com.br')]
        ...
        files.download('dataset-clean.csv')
        print("Número de Linhas: {}".format(len(df3_clean)))
        print("Dataset Final: {}".format(len(df3_clean)))
        return print("acabou")
